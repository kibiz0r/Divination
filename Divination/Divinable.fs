...
module NewDivinable =
    ...
    type Divinable () =
        ...
        static member Lambda (parameterName : string, parameterType : IDivinable, body : IDivinable) =
            DivinableLambda (parameterName, parameterType, body) :> IDivinable
...
module Divinable =
    type Divinable<'T> =
        | FSharpExpr of Expr<'T>

    let eval divinable =
        match divinable with
        | FSharpExpr e ->
            e |> QuotationEvaluator.Evaluate

    let bind (f : 'a -> Divinable<'b>) (divinable : Divinable<'a>) : Divinable<'b> =
        eval divinable |> f

    type DivineBuilder () =
        member this.Bind (divinable : Divinable<'a>, f : 'a -> Divinable<'b>) : Divinable<'b> =
            bind f divinable

        member this.Return (x : 'a) : Divinable<'a> = 
            printfn "Wrapping a raw value into an Divinable"
            Divinable.FSharpExpr <@ x @>

        member this.ReturnFrom (divinable : Divinable<'a>) : Divinable<'a> = 
            printfn "Returning an option directly"
            divinable

        member this.Quote (expr : Expr<Divinable<'a>>) =
            let rec evilTransform quotation =
                match quotation with
                | DerivedPatterns.SpecificCall <@ (+) @> (None, types, l :: r :: []) -> 
                    <@@ %%l - %%r @@>
                | ExprShape.ShapeVar v -> Expr.Var v
                | ExprShape.ShapeLambda (v,expr) -> Expr.Lambda (v, evilTransform expr)
                | ExprShape.ShapeCombination (o, exprs) -> ExprShape.RebuildShapeCombination (o, List.map evilTransform exprs)
            let tExpr = expr |> evilTransform
            let r : Expr<Divinable<'a>> = tExpr |> Expr.Cast
            r

        member this.Run (expr : Expr<Divinable<'a>>) =
            do expr |> printfn "expr %A"
            let newExpr = expr |> this.Quote
            do newExpr |> printfn "newExpr: %A"
            newExpr |> QuotationEvaluator.Evaluate
